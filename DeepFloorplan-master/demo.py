...
parser.add_argument('--im_path', type=str, default='./demo/ours',
                    help='input image paths.')
parser.add_argument('--out_path', type=str, default='./demo/output/',
...
def main(args):
	...
	for root, dirs, files in os.walk(args.im_path, topdown=False):
		for name in files:
			image_path = args.im_path + '/' + name
			...
			with tf.Session(config=tf.ConfigProto(allow_soft_placement=True, log_device_placement=True)) as sess:
				...
				if False :
					...
					path_out = args.out_path + name
					ima_out = cv.cvtColor(floorplan_rgb.astype(np.uint8), cv.COLOR_RGB2BGR)
					cv.imwrite(path_out, ima_out)
		    
	"""
	tf.group(tf.compat.v1.global_variables_initializer(), tf.compat.v1.local_variables_initializer())
	saver = tf.train.import_meta_graph('./pretrained/pretrained_r3d.meta')
	saver.load_weights('./pretrained/pretrained_r3d')
    """
