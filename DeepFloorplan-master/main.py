...
parser.add_argument('--phase', type=str, default='Train', help='Train/Test network.')
...
class MODEL(Network):
	...
	def __init__(self):
		...
		self.eval_file = './dataset/HON_train.txt'		
	...
	def train(self, loader_dict, num_batch, max_step=40000):
		...
		max_ep = NUM_EPOCH #max_step // num_batch
		...
		with tf.Session(config=tf.ConfigProto(allow_soft_placement=True, log_device_placement=True)) as sess:
			...
			for ep in range(max_ep): # epoch loop
				for n in range(num_batch): # batch loop
					...
					# write log 
					print( '[{}/{}] - step {}: loss = {:.3}; {:.2} data/sec, time: {} minutes'.format(ep+1, NUM_EPOCH, n+1, loss_value, 1.0/duration, int(total_times/60)))
	...
	def infer(self, save_dir='out', resize=True, merge=True):
		...
		for p in paths:
			...
			im_x = im.astype(np.float32)
			im_x = cv.resize(im_x, (512,512))/ 255.
			...
			out1_rgb = cv.cvtColor(out1_rgb.astype(np.uint8), cv.COLOR_RGB2BGR)
			out2_rgb = cv.cvtColor(out2_rgb.astype(np.uint8), cv.COLOR_RGB2BGR) 
			out3_rgb = cv.cvtColor(out2_rgb.astype(np.uint8), cv.COLOR_RGB2BGR)  
			

			cv.imwrite(save_path1, cv.cvtColor(out1_rgb, cv.COLOR_RGB2BGR))
			cv.imwrite(save_path2, cv.cvtColor(out2_rgb, cv.COLOR_RGB2BGR))
			if merge:
				cv.imwrite(save_path3, cv.cvtColor(out3_rgb, cv.COLOR_RGB2BGR))
	...
	def evaluate(self, sess, epoch, num_of_classes=11):
		...
		for i in range(n):
			...
			"""
			im = cv.imread(image_paths[i])
			im = cv.cvtColor(im, cv.COLOR_BGR2RGB) 
			dd = cv.imread(gt2_paths[i], 0)
			rr = cv.imread(gt3_paths[i])
			rr = cv.cvtColor(rr, cv.COLOR_BGR2RGB) 
			cw = cv.imread(gt4_paths[i], 0)
			
			im = im.astype(np.float32)
			im = cv.resize(im, (512,512))/ 255.
			im = np.reshape(im, (1,512,512,3))
			
			rr = cv.resize(rr, (512,512))
			rr_ind = rgb2ind(rr)
			
			cw = cw.astype(np.float32)
			cw = cv.resize(cw, (512,512))/ 255.
			dd = dd.astype(np.float32)
			dd = cv.resize(dd, (512,512))/ 255.
		...
		with open('EVAL_'+self.log_dir, 'a') as f:
			...
			for i in range(mean_acc.shape[0]):
				if i not in [7 ,8]: # ingore class 7 & 8 
					f.write( '\nepoch {}: {}th label: accuracy = {:.4}'.format(epoch, i, mean_acc[i]))
			f.write('\n')
		"""
