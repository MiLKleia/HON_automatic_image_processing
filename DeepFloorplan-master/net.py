...
#from tensorflow.compat.v1.slim.nets import vgg
...
def data_loader_bd_rm_from_tfrecord(batch_size=1):
	paths = open('./dataset/HON_train.txt', 'r').read().splitlines()
	#paths = open('./dataset/r3d_train.txt', 'r').read().splitlines()

	loader_dict = read_bd_rm_record('./dataset/HON_train.tfrecords', batch_size=batch_size, size=512)
