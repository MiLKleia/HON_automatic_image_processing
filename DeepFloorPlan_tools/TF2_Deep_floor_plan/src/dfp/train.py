...
def main(config: argparse.Namespace):
    #print(config.logdir, config.logdir + "/G")
    #if not os.path.exists(config.logdir + "/G"):
    #    os.makedirs(config.logdir + "/G")  
    # initialization
    ...
    for epoch in range(config.epochs):
        ...
        #test = convert_one_hot_to_image(logits_r)[0].numpy()
        #print(np.unique(test))
        #cv.imwrite('test.png', test.astype(np.uint8))
        
        # save model
        if epoch % config.save_model_interval == 0:
            model.save_weights(config.logdir + os.sep + "G")
