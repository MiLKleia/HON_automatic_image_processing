...
def load_raw_images(path):
	...
	wall = cv.resize(wall, (512,512))
	close = cv.resize(close, (512,512))
	close_wall = cv.resize(close_wall, (512,512))
	room = cv.resize(room, (512,512))
...
def load_bd_rm_images(path):
	...
	# plt.imshow(bd/256.)
	# plt.show()
	#print(len(np.unique(room_ind)), len(np.unique(cw_ind)))
